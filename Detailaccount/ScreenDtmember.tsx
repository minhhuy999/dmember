...
const ScreenDtmember = ({ route }: any) => {
    ...
    return (
        <View style={styles.backgr}>
            ...
            <Text style={{ textAlign: 'right', color: 'red' }} onPress={()=>console.log(item)}>Xóa</Text>
            <View style={{ width: '100%', height: 240, backgroundColor: 'white', marginTop: 55, borderRadius: 20, alignItems: 'center' }}>
                {item.avatar == "" ?
                    <View style={{ width: 115, height: 115, borderRadius: 120, position: 'absolute', top: -55, backgroundColor: color.bluelow, alignItems: 'center', justifyContent: 'center' }}>
                        <Text style={{ fontWeight: 'bold' }}>{abbreviatedName}</Text>
                    </View> :
                    <Image source={{ uri: item.avatar }} style={{ width: 115, height: 115, borderRadius: 120, position: 'absolute', top: -55 }} />}
                <View style={{ marginTop: 70, alignItems: 'center' }}>
                    <Text style={{ color: 'black', fontSize: 17, fontWeight: '600' }}>{item.fullname}</Text>
            ...
            <View style={{ width: '100%', height: 280, backgroundColor: 'white', borderRadius: 20, paddingHorizontal: 30, paddingVertical: 20 }}>
                ...
                <View style={{ marginBottom: 15 }}>
                    <Text>Giới tính</Text>
                    <Text style={styles.Text1}>{item.sex == 1 ? 'Nam' : 'Nu'}</Text>
                </View>
            </View>
        </View>
    )
